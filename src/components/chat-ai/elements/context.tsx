...
export const Context = ({ className, maxTokens, usedTokens, usage, modelId, ...props }: ContextProps) => {
  ...
  const usedPercent = safeMax > 0 ? Math.min(PERCENT_MAX, Math.max(0, (displayUsedTokens / safeMax) * PERCENT_MAX)) : 0;
    ...
    modelId && cacheReadsTokens > 0
      ? formatUSDFixed(
          estimateCost({
            ...
            usage: { cacheReads: cacheReadsTokens } as Record<string, unknown>,
          }).totalUSD
        )
      : undefined;
  const cacheWritesCostText =
    modelId && cacheWritesTokens > 0
      ? formatUSDFixed(
          estimateCost({
            modelId,
            usage: { cacheWrites: cacheWritesTokens } as Record<string, unknown>,
          }).totalUSD
        )
      : undefined;
  ...
  if (modelId && reasoningTokens > 0) {
    ...
    reasoningCostText = est && Number.isFinite(est) && est > 0 ? formatUSDFixed(est) : "—";
  }
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          ...
          {...props}
        >
          <span className="font-medium text-muted-foreground hidden">{displayPct}</span>
        ...
        <div className="min-w-[240px] space-y-2">
          ...
          <div className="mt-1 space-y-1">
            {hasUsage && uBreakdown.cacheReads && uBreakdown.cacheReads > 0 && (
              <InfoRow label="Cache Hits" tokens={uBreakdown.cacheReads} costText={cacheReadsCostText} />
            )}
            {hasUsage && uBreakdown.cacheWrites && uBreakdown.cacheWrites > 0 && (
              <InfoRow label="Cache Writes" tokens={uBreakdown.cacheWrites} costText={cacheWritesCostText} />
            )}
            <InfoRow label="Input" tokens={displayInput} costText={inputCostText} />
            <InfoRow label="Output" tokens={displayOutput} costText={outputCostText} />
            <InfoRow
              label="Reasoning"
