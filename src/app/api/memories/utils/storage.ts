...
export async function uploadFileToStorage(
  ...
  if (storageBackend === 's3') {
    fatLogger.info('☁️ Using S3 storage backend for file:', 'be', {
      fileName: file.name,
    });
    ...
    try {
      ...
      const s3File = new File([new Uint8Array(buffer)], cleanFileName, {
        type: file.type,
      });
      ...
      fatLogger.error('❌ S3 upload error:', 'be', {
  ...
  fatLogger.info('☁️ Using Vercel Blob storage for file:', 'be', {
    fileName: file.name,
  });
  try {
    ...
    fatLogger.error('❌ Vercel Blob upload error:', 'be', {
      data: error instanceof Error ? error : undefined,
    });
    throw new Error(
...
export async function uploadFileToStorageWithErrorHandling(
  ...
  try {
    fatLogger.info(`📤 Starting ${storageBackend} file upload for:`, 'be', {
      fileName: file.name,
    });
    const url = await uploadFn(file, buffer, storageBackend, userId);
    fatLogger.info(
      `✅ File uploaded successfully to ${storageBackend}:`,
      'be',
      { url }
    );
    return { url, error: null };
  } catch (uploadError) {
    fatLogger.error(`❌ ${storageBackend} upload error:`, 'be', {
      data: uploadError,
    });
    return {
      url: null,
