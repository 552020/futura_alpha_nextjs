...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.log(
      `[${requestId}] Processing commit request for user ${userId}, memory ${memoryId}, ${assets?.length || 0} assets`
    );

    if (!memoryId || !assets || !Array.isArray(assets) || assets.length === 0) {
      console.error(
        `[${requestId}] Invalid commit request - memoryId: ${memoryId}, hasAssets: ${!!assets}, assetCount: ${assets?.length || 0}`
      );
    ...
    const createdAssets = await db.transaction(async tx => {
      ...
      for (const [index, asset] of assets.entries()) {
        ...
        try {
          ...
          console.log(
            `[${requestId}] Creating asset ${index + 1}/${assets.length}: ${asset.name} (${asset.type}, ${asset.size} bytes)`
          );

          const memoryAsset = await tx
            .insert(memoryAssets)
            .values({
              id: uuidv4(),
              memoryId,
          ...
          console.log(
            `[${requestId}] Created asset ${index + 1}/${assets.length} in ${Date.now() - assetStartTime}ms: ${memoryAsset[0]?.id}`
          );
        } catch (error) {
    ...
    console.log(
      `[${requestId}] Successfully committed ${createdAssets.length} assets to memory ${memoryId} in ${Date.now() - startTime}ms`
    );
