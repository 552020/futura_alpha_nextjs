...
export async function GET(
  ...
  try {
    ...
    fatLogger.info('Fetching business relationships for user', 'be', {
      userId,
    });
    ...
    fatLogger.debug('Retrieved business relationships from database', 'be', {
      relationships,
    });

    // Check if the user is a client (has a business)
    const clientRelationships = relationships.filter(
      (rel) => rel.clientId === userId
    );
    const isClient = clientRelationships.length > 0;

    // Check if the user is a business (has clients)
    const businessRelationships = relationships.filter(
      (rel) => rel.businessId === userId
    );
    ...
    if (isClient) {
      fatLogger.debug('Client relationships found', 'be', {
        clientRelationships,
      });
      ...
      const businessEmails = await Promise.all(
        clientRelationships.map(async (rel) => {
          try {
            fatLogger.debug('Looking up business user', 'be', {
            ...
            if (!businessUser) {
              fatLogger.warn(`Business user not found in users table`, 'be', {
                businessId: rel.businessId,
              });
              return null;
            }
            ...
            if (!businessUser.email) {
              fatLogger.warn('Business user has no email set', 'be', {
                businessId: rel.businessId,
              });
              return null;
            }
            ...
            fatLogger.error('Error fetching business user', 'be', {
              businessId: rel.businessId,
              error,
            });
            return null;
      ...
      fatLogger.debug('Business emails found for client', 'be', {
        businessEmails,
      ...
      if (!businessEmail) {
        fatLogger.warn(
          'No business email found, listing all users for debugging',
          'be'
        );
