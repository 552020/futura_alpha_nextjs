...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!galleriesResult.success || !galleriesResult.data) {
      return NextResponse.json(
        { error: galleriesResult.error || 'Failed to fetch galleries' },
        { status: 500 }
      );
    }
    ...
    const galleriesWithStorageStatus =
      await addStorageStatusToGalleries(paginatedGalleries);
    ...
    fatLogger.error('Error listing galleries:', 'be', {
  ...
  try {
    ...
    if (type === 'from-folder') {
      ...
      fatLogger.info('🔍 All folders with name:', 'be', {
        ...
        folders: allFoldersWithName.map((f) => ({
          id: f.id,
          name: f.name,
      ...
      const folderMemories = await db.query.memories.findMany({
        where: and(
          eq(memoriesTable.ownerId, allUserRecord.id),
          eq(memoriesTable.parentFolderId, folder.id)
        ),
      });

      fatLogger.info('🔍 Found folder memories:', 'be', {
        count: folderMemories.length,
        memories: folderMemories.map((m) => ({
          id: m.id,
          title: m.title,
    ...
    const { createGalleryRecord, createGalleryItems } = await import(
      '@/services/gallery/gallery-operations'
    );
    ...
    const galleryResult = await createGalleryRecord({
      ...
      title:
        title ||
        (type === 'from-folder' ? `Gallery from ${folderName}` : 'My Gallery'),
    ...
    if (!galleryResult.success || !galleryResult.data) {
      return NextResponse.json(
        { error: galleryResult.error || 'Failed to create gallery' },
        { status: 500 }
      );
    }
    ...
    if (!itemsResult.success) {
      fatLogger.warn('Failed to create some gallery items', 'be', {
