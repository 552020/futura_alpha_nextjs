...
export async function PUT(
  ...
  fatLogger.info('[Folders API] Incoming update', 'be', {
    id,
    body: { title, name },
  });
  ...
  if (!allUserRecord) {
    fatLogger.error('[Folders API] No allUsers record', 'be', {
  ...
  if (!existing) {
    fatLogger.error(
      '[Folders API] Folder not found or not owned by user',
      'be',
      { id, ownerId: allUserRecord.id }
    );
  ...
  if (!updateResult.success || !updateResult.data) {
    fatLogger.error('[Folders API] Failed to update folder', 'be', {
      id,
  ...
  fatLogger.info('[Folders API] Folder updated', 'be', {
    id,
    updated: { title: updateResult.data.title, name: updateResult.data.name },
  });
