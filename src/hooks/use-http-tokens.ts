...
export function useDashboardAssets() {
  ...
  const loadThumbnails = useCallback(
    async (
      ...
      try {
        fatLogger.info('Loading dashboard thumbnails', 'fe', {
        ...
        const neonMemories = memories.filter(
          (memory) => !icpMemories.includes(memory)
        );
  ...
  const loadPreviews = useCallback(
    async (memoryIds: string[]): Promise<Map<string, string>> => {
      try {
        fatLogger.info('Loading dashboard previews', 'fe', {
  ...
  const loadAllVariants = useCallback(
    async (
      ...
      try {
        fatLogger.info('Loading all dashboard variants', 'fe', {
          count: memories.length,
        });
