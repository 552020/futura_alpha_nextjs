import { pgTable, uniqueIndex, text, timestamp, foreignKey, uuid, integer, unique, boolean, index, json, check, bigint, serial, varchar, jsonb, primaryKey, pgEnum } from "drizzle-orm/pg-core"
...
export const gallery = pgTable("gallery", {
	...
	storageLocations: storageLocationT("storage_locations").array().default([""]),
	averageStorageDuration: integer("average_storage_duration"),
	storageDistribution: json("storage_distribution").default({}),
}, (table) => [
	index("galleries_storage_duration_idx").using("btree", table.averageStorageDuration.asc().nullsLast().op("int4_ops")),
...
export const memoryComments = pgTable("memory_comments", {
	...
	index("memory_comments_memory_created_idx").using("btree", table.memoryId.asc().nullsLast().op("uuid_ops"), table.createdAt.desc().nullsLast().op("timestamp_ops")),
...
export const memoryLikes = pgTable("memory_likes", {
	...
	uniqueIndex("memory_likes_unique").using("btree", table.memoryId.asc().nullsLast().op("uuid_ops"), table.allUserId.asc().nullsLast().op("uuid_ops")),
...
export const peopleInMemories = pgTable("people_in_memories", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	memoryId: uuid("memory_id").notNull(),
	allUserId: text("all_user_id").notNull(),
	...
	uniqueIndex("people_in_memories_unique").using("btree", table.memoryId.asc().nullsLast().op("uuid_ops"), table.allUserId.asc().nullsLast().op("uuid_ops")),
...
export const serviceDeployments = pgTable("service_deployments", {
	...
	index("service_deployments_user_active_idx").using("btree", table.userId.asc().nullsLast().op("text_ops"), table.isActive.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [user.id],
...
export const memories = pgTable("memories", {
	...
	index("memories_owner_created_idx").using("btree", table.ownerId.asc().nullsLast().op("text_ops"), table.createdAt.desc().nullsLast().op("text_ops")),
