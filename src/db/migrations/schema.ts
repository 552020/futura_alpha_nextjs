...
export const storageEdges = pgTable(
  ...
  table => [
    index('ix_edges_backend_present').using(
      'btree',
      table.backend.asc().nullsLast().op('bool_ops'),
      table.artifact.asc().nullsLast().op('enum_ops'),
      table.present.asc().nullsLast().op('enum_ops')
    ),
    index('ix_edges_memory').using(
      'btree',
      table.memoryId.asc().nullsLast().op('uuid_ops'),
      table.memoryType.asc().nullsLast().op('uuid_ops')
    ),
    index('ix_edges_sync_state').using('btree', table.syncState.asc().nullsLast().op('enum_ops')),
    uniqueIndex('uq_edge').using(
      'btree',
      table.memoryId.asc().nullsLast().op('enum_ops'),
      table.memoryType.asc().nullsLast().op('uuid_ops'),
      table.artifact.asc().nullsLast().op('enum_ops'),
      table.backend.asc().nullsLast().op('uuid_ops')
    ),
  ]
);
